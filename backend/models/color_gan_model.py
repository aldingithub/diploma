...
class ColorGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # define networks
        self.net_cycleGAN = networks_color.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, 
                                        opt.resnet_n_downsample)

        netG_A_decoder = self.net_cycleGAN.decoder
        self.netG_A = networks_color.define_AdaINNet(self.gpu_ids, netG_A_decoder)
        ...
        if self.isTrain:  # define discriminators
            print('build:netD_A')
            self.netD_A = networks_color.define_D(opt.output_nc, opt.ndf, opt.netD,
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks_color.GANLoss(opt.gan_mode).to(self.device)  # define GAN loss.
